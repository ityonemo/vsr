defmodule VsrServer do
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      ...
      @doc false
      def handle_commit(_operation, state, _vsr_state) do
        {state, :ok}
      end
  ...
  """
  @callback handle_commit(operation :: term, inner_state :: term, vsr_state :: term) ::
    ...
    cond do
      ...
      client_id && request_id ->
        case Map.fetch(state.client_table, client_id) do
          ...
          {:ok, {:cached, cached_request_id, cached_result}} when request_id < cached_request_id ->
  ...
  defp process_new_client_request(state, operation, from, client_id, request_id) do
    ...
    updated_state = if client_id && request_id do
      ...
      %{new_state | client_table: Map.put(new_state.client_table, client_id, {:processing, request_id})}
    else
      new_state
    end
    ...
    if new_commit_number > state.commit_number do
      ...
      {new_inner, operation_results} =
        Enum.reduce(log_entries, {state.inner, []}, fn entry, {inner_acc, results_acc} ->
          {new_inner, result} = apply_operation(state, inner_acc, entry.operation)
  ...
  defp apply_operation(state, inner_state, operation) do
    if function_exported?(state.module, :handle_commit, 3) do
      state.module.handle_commit(operation, inner_state, state)
    else
      {inner_state, :ok}
    end
  end
