defmodule HeartbeatTest do
  ...
  defp start_replica(_id) do
    {:ok, pid} = GenServer.start_link(Vsr, [
      ...
      state_machine: VsrKv,
      cluster_size: 3,
      heartbeat_interval: 100,      # Fast heartbeat for testing
      primary_inactivity_timeout: 300  # Fast timeout for testing
    ])
    {:ok, pid}
  end
  ...
  test "primary should send heartbeats to replicas", %{replicas: [primary, backup1, backup2]} do
    ...
    states = Enum.map([primary, backup1, backup2], &Vsr.dump/1)
    primary_replica = Enum.find([primary, backup1, backup2], fn replica ->
  ...
  test "backup should detect primary failure and trigger view change", %{replicas: [replica1, _replica2, _replica3]} do
  ...
  test "heartbeat should reset primary inactivity timer", %{replicas: [_replica1, replica2, _replica3]} do
  ...
  test "primary inactivity should trigger automatic view change" do
    ...
    {:ok, backup} = GenServer.start_link(Vsr, [
      log: [],
      state_machine: VsrKv,
      cluster_size: 3,
      heartbeat_interval: 50,
      primary_inactivity_timeout: 150  # Very short timeout for testing
    ])
