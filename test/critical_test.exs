defmodule CriticalTest do
  ...
  test "quorum calculation should use cluster_size consistently", %{
    ...
    # +1 for self
    connected_count1 = MapSet.size(state1.replicas) + 1
  ...
  test "client requests should be deduplicated", %{replicas: [replica1, _replica2, _replica3]} do
    ...
    # Should be same key
    result2 = VsrKv.put(replica1, "dedup_test", "value2")
    ...
    assert {:ok, "value2"} = VsrKv.fetch(replica1, "dedup_test")
    # Shows no deduplication - second request processed
  end

  # Test 4: Sequential operation validation 
  test "operations should be sequential without gaps", %{
  ...
  test "prepare_ok_count should be cleaned up after commits", %{
    replicas: [replica1, _replica2, _replica3]
  } do
    ...
    stale_entries =
      Enum.filter(older_committed_ops, fn op_num ->
    ...
    assert stale_entries == [],
           "Found stale prepare_ok_count entries for older committed ops: #{inspect(stale_entries)}"
