defmodule QuorumTest do
  ...
  test "quorum calculation should be consistent with cluster_size" do
    ...
    {:ok, replica} = GenServer.start_link(Vsr, [
      log: [],
      state_machine: VsrKv,
      cluster_size: 5  # Total cluster size
    ])
  ...
  test "quorum function should use cluster_size consistently" do
    ...
    connected_count = MapSet.size(state.replicas) + 1  # 3
    cluster_size = state.cluster_size                  # 5
    
    # The correct quorum check should be: connected_count > cluster_size / 2
    expected_has_quorum = connected_count > div(cluster_size, 2)  # 3 > 2 = true
