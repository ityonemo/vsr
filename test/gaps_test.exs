defmodule GapsTest do
  ...
  test "should reject prepare messages with gaps in operation numbers", %{primary: primary, backup: backup} do
    ...
    gap_prepare = %Vsr.Message.Prepare{
      ...
      op_number: initial_log_length + 2,  # This creates a gap
  ...
  test "should trigger state transfer when gap is detected", %{primary: primary, backup: backup} do
    ...
    gap_prepare = %Vsr.Message.Prepare{
      view: backup_state.view_number,
      op_number: backup_state.op_number + 5,  # Big gap
    ...
    log_ops = Enum.map(final_backup_state.log, & &1.operation)
    refute {:put, "far_key", "far_value"} in log_ops, "Gap operation should not be in log without state transfer"
